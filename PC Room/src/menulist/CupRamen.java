...
public class CupRamen extends JPanel {
	...
	int count = 0;
	String show = "";
	String menu[] = { "진라면", "불닭볶음면", "신라면" };
	int price[] = { 2000, 2500, 2000 };
	...
	JTable table;
	ImageIcon[] img = { new ImageIcon("Image/cup1.png"), // 사진 캡쳐할때 일러스트 확대크기25%
			new ImageIcon("Image/cup2.png"), new ImageIcon("Image/cup3.png"), new ImageIcon("Image/no.png"),
			new ImageIcon("Image/no.png"), new ImageIcon("Image/no.png"), new ImageIcon("Image/no.png"),
...
// "+" 버튼 이벤트
			...
			ok[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					/*
					 * for (int i = 0; i<me.table.getRowCount(); i++){ //int amount =
					 * Integer.parseInt(me.table.getValueAt(i, 3)+""); //total += amount; }
					 */
					
					//LabelResult.setText("총합의 금액 = " + Integer.toString(sum));
					//MainMenu.CupRamen(sum);
					
					//System.out.println(LabelResult.getText());
					String menuDB[] = { "JinRamen", "BullDack", "SinRamen" };

					// String InsertQuery = "insert into stock(Coke) Values(" + count + ");";
					String selectQuery = "update stock set Coke = Coke-" + count + "where Coke > " + count + ";";

					String select1Query = "update stock set " + menuDB[j] + "=" + menuDB[j] + "-" + count + "where "
							+ menuDB[j] + ">" + count + ";";
					...
					String[] order = { menu[j], String.valueOf(price[j]), String.valueOf(count),
							String.valueOf(price[j] * count) };
	...
	public void CupRemen(int a, DataOutputStream out) {
		PcNum = a;
		Queryout = out;
	}
}